...
class UserManager(models.Manager):
    def register(self,postData):
        ...
        if not postData['first_name'] or postData['first_name'] < 3:
            result['status'] = False
            result['error'].append('First name mut be 3 characters or longer')
        if not postData['last_name'] or postData['last_name'] < 3:
            result['status'] = False
            result['error'].append('Last name mut be 3 characters or longer')
        ...
        if not postData['password'] or postData['password'] < 8:
            result['status'] = False
            result['error'].append('Password must be 8 characters or longer')
